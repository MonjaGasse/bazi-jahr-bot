...
async def handle_birthday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Visuellen Text hinzufügen
    await update.message.reply_text(
        f"🎉✨ **Ich habe dein Geburtsdatum erhalten:** {birth_date}\n\n"
        "🔮 Jetzt wird deine *Bazi Jahressäule* für dich gelesen... bitte einen Moment Geduld. 🌟",
    ...
    if not chinese_year:
        await update.message.reply_text(
            "❌ Das Datum scheint ungültig zu sein. Bitte überprüfe, ob du das Format **'TT.MM.JJJJ, HH:MM'** korrekt eingegeben hast. 🧐",
            parse_mode=ParseMode.MARKDOWN
        )
        return
...
await update.message.reply_text(
        f"✨ Dein Jahr im chinesischen Kalender: **{heavenly_stem} {earthly_branch}**\n\n"
        f"📝 **Beschreibung:** {description}\n\n"
        "💫 Möge dieses Jahr dir Weisheit und Wachstum bringen! 🌱",
        parse_mode=ParseMode.MARKDOWN
    )
...
if __name__ == "__main__":
    ...
    app.run_polling(poll_interval=1, allowed_updates=["message"])
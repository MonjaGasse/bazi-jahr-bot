...
async def handle_birthday(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🎉 Geburtsdatum erhalten: {birth_date}. Jetzt wird deine Bazi Jahrressäule gelesen 🥰",
    parse_mode=ParseMode.HTML
)
    ...
    print(f"Himmelsstamm: {heavenly_stem}, Erdzweig: {earthly_branch}, Beschreibung: {description}") # Debugging
     f"🌟 Das Jahr {chinese_year} im chinesischen Kalender ist: {heavenly_stem} {earthly_branch}. {description} ✨",
        parse_mode=ParseMode.HTML
    )
